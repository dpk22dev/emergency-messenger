...
public class CheckBoxifiedTextListAdapter extends BaseAdapter {
     ...
     public void setChecked(boolean value, int position){
    	 Toast.makeText(mContext, " checked at " + position 
		          , Toast.LENGTH_SHORT).show();
         mItems.get(position).setChecked(value);
     }
     
     public boolean getChecked(int position){
    	 Toast.makeText(mContext, " item " +  mItems.get(position).getText() + " status " + mItems.get(position).getChecked() 
		          , Toast.LENGTH_SHORT).show();
    	 
         return mItems.get(position).getChecked();
     }
